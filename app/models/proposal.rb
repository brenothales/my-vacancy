class Proposal < ActiveRecord::Base
  ...
  scope :has_new_proposals, lambda { |user| 
    joins(:announcement).where('proposals.situation = ? AND announcements.user_id = ?', false, user.id).group('proposals.announcement_id')
  }
  ...
  scope :unread, lambda { |user|
    if user.is_role? :administrador
      ...
      joins(:announcement).where('proposals.situation = ? AND announcements.user_id = ?', false, user.id) 
    end
  }
  ...
  scope :read, lambda { |user|
    if user.is_role? :administrador
      ...
      joins(:announcement).where('proposals.situation = ? AND announcements.user_id = ?', true, user.id) 
    end
  }
  ...
  def self.search(search, order_by, ordem)
    unless search.nil? || search.empty?
      joins(:announcement).where('proposals.name LIKE :search OR proposals.content LIKE :search OR announcements.name = :search', :search => "%#{search}%").order("#{order_by} #{ordem}")
    else
